...
生产构建必须显式提供三个互不相同的 HTTPS 地址：

```bash
TARO_APP_REVIEW_API_BASE_URL=https://review-api.example.com \
TARO_APP_STAGING_API_BASE_URL=https://staging-api.example.com \
TARO_APP_PRODUCTION_API_BASE_URL=https://api.example.com \
yarn build:weapp
```

`TARO_APP_API_BASE_URL` 只作为本地开发兼容值，生产构建不会用它替代上述三个地址。提审前需将三个 HTTPS 域名全部加入小程序合法请求域名。
