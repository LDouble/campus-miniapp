...
小程序在同一份提审代码内固化两个 API 地址。当前版本所有运行环境统一使用 production API：
...
未知值同样使用 production；不支持通过远程配置、请求 Header 或查询参数切换 API 环境。
...
`TARO_APP_API_BASE_URL` 只作为兼容值，实际请求默认使用 `product.weouc.com`；如需覆盖 production API，使用 `TARO_APP_PRODUCTION_API_BASE_URL`。review 地址仍作为构建隔离配置保留，但当前运行时不会请求它。
