...
- 文本相关 Token 使用固定像素，以小米 14 Ultra 约 393px 的逻辑屏幕宽度为参考，按 `1rpx ≈ 393 / 750 ≈ 0.524px` 保留现有设计值；源码中的固定单位写作 `PX`，用于绕过 Taro `postcss-pxtransform`，运行时仍等价于 CSS `px`。横向布局、图片、触控热区和安全区继续使用 `rpx` 或弹性布局。
...
| 层级 | 字号 | 字重 | 建议行高 | 场景 |
| --- | --- | --- | --- | --- |
| `badge` | `10.48px`（原 `20rpx`） | `400–700` | `14.67px` | 徽章、角标 |
| `caption` | `12.58px`（原 `24rpx`） | `400–600` | `17.61px` | 时间与辅助信息 |
| `label` | `14.15px`（原 `27rpx`） | `400–600` | `19.81px` | 昵称、操作按钮 |
| `comment` | `15.72px`（原 `30rpx`） | `400` | `25.94px` | 评论和信息流摘要 |
| `body` | `16.77px`（原 `32rpx`） | `400` | `31.02px` | 帖子正文 |
| `title` | `17.29px`（原 `33rpx`） | `500–600` | `24.21px` | 模块标题 |
...
Ousea / Global 的横向间距继续使用 `rpx`/弹性布局；卡片和模块的纵向节奏按 393px 逻辑宽度换算后固定为 `PX`，避免随屏宽发生比例变化。
...
| --- | --- | --- |
| `block-1` | `8.38px` | 紧凑间距 |
| `block-2` | `12.58px` | 卡片内间距 |
| `block-3` | `16.77px` | 模块间距 |
| `block-4` | `20.96px` | 页面纵向间距 |
| `block-5` | `33.54px` | 大区块分隔 |

- 页面水平边距优先使用 `28rpx` 或 `32rpx`，同一页面不得混用多套主边距。
- 列表卡片纵向间距使用固定 `8.38–16.77px`；常用服务宫格应保持紧凑，不为填满高度强行拉大行距。
- 相邻触控目标之间至少保留 `16rpx`。
...
- 图标操作区应覆盖可见图形及必要内边距，不使用透明热区把卡片或列表行额外撑高；上下留白由组件自身的固定间距控制。
...
- 以 `320px`、`360px`、`393px` 宽度至少各验证一次，不允许横向溢出。
...
- 主要触控区域应覆盖可见控件和必要内边距，不依赖系统默认的 `min-block-size` 撑开布局；相邻触控目标间距不小于 `16rpx`。
...
- [ ] 320–393px 窄屏无横向溢出，长文本有合理换行或截断。
- [ ] 微信胶囊、吸顶区、键盘和安全区不会遮挡内容。
- [ ] 触控区域与可见控件及上下间距匹配，点击态不引发布局位移。
