...
本次新增一个向后兼容的可选 `image_state` 响应字段，不改变现有图片上传、私信发送和审核接口路径，也不删除旧版 `image` 字段。新服务端允许 ready 但尚未通过审核的私信图片先绑定消息，并在消息创建事务中排队审核；客户端不主动查询审核状态，状态更新依赖后续消息刷新。旧版服务端若拒绝待审图片，客户端单次提交审核并保留手动重试入口。
...
3. 新服务端在图片消息创建事务中排队审核，客户端不启动媒体审核轮询；对仍拒绝待审媒体的旧服务端，客户端调用一次私信图片审核提交接口，若接口立即返回通过则重发，否则保留手动重试。审核过程不阻塞用户继续输入。
...
2. **审核异步化**
   - 审核提交发生在私信消息创建成功之后。
   - 客户端不主动轮询审核，不阻塞聊天输入、滚动或新消息轮询。
   - 审核失败不能再次创建同一图片消息；旧服务端仅允许手动重试审核提交或更换图片。
...
- `src/packages/social/direct-messages/chat.tsx` - 图片自动发送、旧接口手动兼容和失效占位。
...
- 发送接口若在媒体尚未提交审核时拒绝 `media_id`，客户端必须回退到现有审核接口；接口立即返回通过时用新幂等键重发，否则保留手动重试，不能静默丢图。
- 审核提交失败不能被当作审核拒绝；应保留手动重试状态，不启动后台轮询。
