...
- 校方返回 `academic_challenge_required` 时进入当前页面会话 30 分钟冷却；冷却期间再次提交不发请求，并提示剩余等待时间。
...
4. **验证码冷却**
   - 首次出现验证码或设备确认时，明确提示用户等待 30 分钟后再次尝试。
   - 当前页面会话内保存不含账号密码的冷却截止时间。
   - 冷却期内再次点击验证不请求后端，弹窗展示剩余分钟数。
   - 冷却到期后自动允许再次验证；验证码不记为错误密码。
   - 离开并重新进入绑定页面后不保留冷却，允许立即再次提交。
...
- `src/features/academic-verification/challenge-cooldown.ts` - 封装 30 分钟验证码冷却时间计算。
