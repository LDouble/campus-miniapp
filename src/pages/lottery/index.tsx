...
const stateLabel = (item: LotteryCampaignSummary) => {
  ...
  if (item.status === 'closed') return item.draw_mode === 'scheduled' ? '等待开奖' : '活动已结束'
...
export default function LotteryCampaignListPage() {
  ...
  return (
    <View className='lottery-list-page'>
      <CustomNavbar title='校园抽奖' subtitle='认证参与，收获一份校园惊喜' showBack />
      <View className='lottery-list-page__content'>
        ...
        {items.map((item) => (
          <View
            key={item.id}
            className={`lottery-campaign-card${['结果已公布', '活动已结束', '活动已取消'].includes(stateLabel(item)) ? ' lottery-campaign-card--ended' : ''}`}
            ariaRole='button'
            ariaLabel={`查看${item.title}`}
            onClick={() => void Taro.navigateTo({ url: `/pages/lottery/detail?id=${item.id}` })}
          >
            <View className='lottery-campaign-card__main'>
            <Image className={`lottery-campaign-card__cover${!item.cover_url || failedImages[item.id] ? ' lottery-campaign-card__cover--placeholder' : ''}`} src={failedImages[item.id] ? giftImage : item.cover_url || giftImage} mode={!item.cover_url || failedImages[item.id] ? 'aspectFit' : 'aspectFill'} onError={() => setFailedImages(current => ({ ...current, [item.id]: true }))} />
            <View className='lottery-campaign-card__body'>
              ...
              <View className='lottery-campaign-card__tags'><Text className='lottery-campaign-card__tag'>校园认证专享</Text><Text>{modeLabel(item.draw_mode)}</Text></View>
              <Text className='lottery-campaign-card__prizes'>
                {item.prizes.slice(0, 3).map((prize) => `${prize.name} ×${prize.total_quantity}`).join(' · ') || '奖品即将公布'}
                {item.prizes.length > 3 ? ` 等 ${item.prizes.length} 种奖品` : ''}
              </Text>
            </View>
            </View>
            <View className='lottery-campaign-card__footer'><View className='lottery-campaign-card__time'><Image src={clockIcon} /><Text>截止 {timeLabel(item.end_at)}</Text></View><Text className='lottery-campaign-card__action'>{item.status === 'completed' ? '查看结果' : stateLabel(item) === '进行中' ? '查看活动' : '查看详情'} ›</Text></View>
          </View>
        ))}
        {loadingMore && <View className='lottery-state lottery-state--more'>正在加载更多活动…</View>}
        {!!error && !!items.length && <View className='lottery-load-more' onClick={() => void load()}>{error} · 点击重试</View>}
        {!loadingMore && items.length < total && <View className='lottery-load-more' onClick={() => void load(false)}>加载更多活动</View>}
        {!loading && !!items.length && items.length >= total && <View className='lottery-list-end'>— 已经到底啦，更多活动敬请期待 —</View>}
      </View>
    </View>
  )
}
