...
assert.match(feedStyles, /\.community-post__more \{[\s\S]*?width:\s*56rpx;[\s\S]*?height:\s*56rpx;/u)
assert.match(feedStyles, /\.community-post__more::before \{[\s\S]*?width:\s*56rpx;[\s\S]*?height:\s*23\.06PX;/u)
assert.match(feedStyles, /\.community-post__action-menu \{[\s\S]*?right:\s*64rpx;[\s\S]*?width:\s*266rpx;[\s\S]*?min-height:\s*72rpx;[\s\S]*?background:\s*transparent;/u)
