...
assert.ok(homePage.includes('refreshPrivateMessageUnreadCount(force)'), '首页必须聚合刷新私信未读数')
...
assert.ok(homePage.includes('getWechatSubscriptionSettings(templateIds)'), '首页必须按模板读取微信订阅设置')
...
assert.ok(homePage.includes('return getMyDailyCheckinStatus()'), '首页必须加载签到状态')
assert.ok(homePage.includes('home-notification-guide'), '首页缺少通知引导浮层')
assert.ok(homePage.includes('setCustomTabBarHidden(show)'), '通知引导出现时必须隐藏 TabBar')
