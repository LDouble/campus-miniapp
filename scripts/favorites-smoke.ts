...
assert.match(profileStyle, /grid-template-columns:\s*repeat\(5,\s*minmax\(0,\s*1fr\)\)/u, '我的服务对齐首页五列服务宫格')
assert.match(profileStyle, /gap:\s*8rpx 4rpx/u, '我的服务宫格沿用首页紧凑间距')
assert.match(profileStyle, /height:\s*132rpx/u, '我的服务入口沿用首页安全触控高度')
assert.match(profileStyle, /width:\s*76rpx;\s*height:\s*76rpx;\s*margin-bottom:\s*8rpx/u, '我的服务图标底板尺寸对齐首页')
assert.match(profileStyle, /background:\s*var\(--ousea-ocean-50,[\s\S]{0,80}border:\s*2rpx solid var\(--ousea-ocean-100/u, '我的服务图标底板使用 Ousea 浅蓝 token')
