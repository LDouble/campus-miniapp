...
assert.equal(resolveApiBaseUrl('trial', endpoints), 'https://staging-api.example.invalid')
...
assert.deepEqual(loadApiEndpoints({
  ...
  TARO_APP_STAGING_API_BASE_URL: endpoints.staging,
...
assert.throws(() => loadApiEndpoints({
  ...
  TARO_APP_STAGING_API_BASE_URL: endpoints.review,
  TARO_APP_PRODUCTION_API_BASE_URL: endpoints.production,
